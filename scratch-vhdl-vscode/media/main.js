...
document.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener(
    'message',
    (message) =>
      message.data.type == 'getState' &&
      requests[message.data.id](message.data.value)
  );
  ...
  window.addEventListener(
    'message',
    (message) => message.data.type == 'setState' && requests[message.data.id]()
  );
  ...
  class StorageBackpack extends Backpack {
    open() {
      ...
      getState('bp_contents', []).then(
        (d) => ((this.contents_ = d), super.open())
      );
    }
    onContentChange_() {
  ...
  Blockly.defineBlocksWithJsonArray([
    ...
    },
    {
      ...
      tooltip:
        'Create a process that is sensitive to all that does `VAR <= VALUE`',
    },
    {
      kind: 'block',
      type: 'value_std_logic',
  ...
  (() => {
    ...
    Blockly.fieldRegistry.register(
      'field_variable',
      (Blockly.FieldVariable = class extends Blockly.FieldDropdown {
        constructor(varName, opt_config) {
        ...
        initModel() {
          ...
          if (!this.var)
            if (this.defaultVariableName === '') {
              this.doValueUpdate_(Object.keys(entity.entity)[0]);
            } else {
              // Don't call setValue because we don't want to cause a rerender.
              this.doValueUpdate_(this.defaultVariableName);
            }
        }
        ...
        loadState(state) {
          ...
          this.setValue(
            state.name ||
              Blockly.getMainWorkspace().getVariableById(state.id).name
          );
        }
        ...
        onItemSelected_(menu, menuItem) {
          ...
          if (this.sourceBlock_ && !this.sourceBlock_.isDeadOrDying()) {
            if (id === 'DELETE_VARIABLE_ID') {
              if (entity.signals[this.var]) delete entity.signals[this.var];
              else if (entity.aliases[this.var]) {
                delete entity.aliases[this.var];
              } else if (entity.entity[this.var]) {
                delete entity.entity[this.var];
              }
              vscode.postMessage({
                type: 'updateEntity',
              ...
              if (entity.signals[this.var]) {
                entity.signals[prompt('New name:', this.var)] =
                  entity.signals[this.var];
                delete entity.signals[this.var];
              } else if (entity.aliases[this.var]) {
                entity.aliases[prompt('New name:', this.var)] =
                  entity.aliases[this.var];
                delete entity.aliases[this.var];
              } else if (entity.entity[this.var]) {
                entity.entity[prompt('New name:', this.var)] =
                  entity.entity[this.var];
                delete entity.entity[this.var];
              }
              vscode.postMessage({
                type: 'updateEntity',
        ...
        static dropdownCreate() {
          ...
          for (let i = 0; i < variableModelList.length; i++) {
            ...
            options[i] = [variableModelList[i], variableModelList[i]];
          }
        ...
        renderSelectedText_() {
          ...
          if (
            ...
            if (Object.keys(entity.entity).indexOf(this.getValue()) !== -1)
              this.sourceBlock_.setColour(20);
            else if (
              Object.keys(entity.aliases).indexOf(this.getValue()) !== -1
            )
  ...
  Generator.controls_case = function (generator, block) {
    return (
      'case ' +
      generator.valueToCode(block, 'ON', generator.ORDER_NONE) +
      ' is\n' +
  ...
  Generator.logic_or = function (generator, block) {
    return [
      generator.valueToCode(block, 'A', generator.ORDER_NONE) +
        ' | ' +
        generator.valueToCode(block, 'B', generator.ORDER_NONE),
      generator.ORDER_ATOMIC,
    ];
  };
  ...
  Generator.logic_not = function (generator, block) {
    return [
      'not ' + generator.valueToCode(block, 'INPUT', generator.ORDER_NOT),
      generator.ORDER_NOT,
    ];
  };
  ...
  Generator.logic_operation = function (generator, block) {
    return [
      generator.valueToCode(block, 'A', generator.ORDER_LOGIC) +
        ' ' +
        block.getFieldValue('OPERATION') +
        ' ' +
        generator.valueToCode(block, 'B', generator.ORDER_LOGIC),
      generator.ORDER_LOGIC,
    ];
  };
  ...
  Generator.logic_operation_vector = function (generator, block) {
    return [
      block.getFieldValue('OPERATION') +
        '(' +
        generator.valueToCode(block, 'LIST', generator.ORDER_NONE) +
        ')',
      generator.ORDER_FUNCTION_CALL,
    ];
  };
  ...
  Generator.logic_compare = function (generator, block) {
    ...
    return [
      generator.valueToCode(block, 'A', generator.ORDER_COMPARE) +
        ' ' +
        OPERATORS[block.getFieldValue('OP')] +
        ' ' +
        generator.valueToCode(block, 'B', generator.ORDER_COMPARE),
      generator.ORDER_COMPARE,
    ];
  };

  Generator.variables_get = function (generator, block) {
    return [block.getField('VAR').getText(), generator.ORDER_ATOMIC];
  };
  ...
  Generator.math_change = function (generator, block) {
    return (
      ...
      block.getField('VAR').getText() +
      ' + ' +
      generator.valueToCode(block, 'DELTA', generator.ORDER_ADD) +
      ';\n'
    );
  };
  ...
  Generator.math_arithmetic = function (generator, block) {
    ...
    const order = tuple[1];
    return [
      (generator.valueToCode(block, 'A', order) || '0') +
        tuple[0] +
        (generator.valueToCode(block, 'B', order) || '0'),
      order,
    ];
  };
  ...
  Generator.list_index = function (generator, block) {
    return [
      generator.valueToCode(block, 'LIST', generator.ORDER_INDEX) +
        '(' +
        generator.valueToCode(block, 'INDEX', generator.ORDER_NONE) +
        ')',
  ...
  Generator.list_concat = function (generator, block) {
    return [
      generator.valueToCode(block, 'A', generator.ORDER_CONCAT) +
        ' & ' +
        generator.valueToCode(block, 'B', generator.ORDER_CONCAT),
      generator.ORDER_CONCAT,
    ];
  };
  ...
  Object.keys(Generator).forEach((b) => {
    ...
    MAPGenerator[b] = function (block) {
      ...
      if (c instanceof Array)
        return [u(0x200b) + block.id + u(0x200b) + c[0] + u(0xfeff), c[1]];
  ...
  function highlightCodePosition(line, column) {
    ...
    ws.getBlockById(
      Object.entries(map)
        .filter(
          ([_, range]) =>
            (range[0][0] < line && range[1][0] > line) ||
            (range[0][0] === line && column > range[0][1]) ||
            (range[1][0] === line && column < range[1][1])
        )
        .reduce(
          (a, b) =>
            a[1][0][0] > b[1][0][0] ||
  ...
  const makeChanges = (changes, c = false) => {
    const renamed = Object.fromEntries(changes.renamed);
    if (c)
      Blockly.getMainWorkspace()
        .getBlocksByType('constant')
        .forEach((block) => {
          if (renamed.hasOwnProperty(block.getField('NAME').getValue())) {
            block
              .getField('NAME')
              .setValue(renamed[block.getField('NAME').getValue()]);
          } else if (
            changes.deleted.includes(block.getField('NAME').getValue())
          ) {
            block.checkAndDelete();
          }
        });
    else
      field_variables.forEach((field_variable) => {
        if (renamed.hasOwnProperty(field_variable.getValue())) {
          field_variable.setValue(renamed[field_variable.getValue()]);
        } else if (changes.deleted.includes(field_variable.getValue())) {
          field_variable.sourceBlock_.checkAndDelete();
        }
      });
  };
  ...
  Blockly.ContextMenuRegistry.registry.register({
    ...
    callback() {
      table_modal(['name', 'type', 'value'], entity.signals).then(
        ({ data, changes }) => {
          entity.signals = data;
          makeChanges(changes);
          vscode.postMessage({
  ...
  Blockly.ContextMenuRegistry.registry.register({
    ...
    callback() {
      table_modal(['name', 'type', 'value'], entity.constants).then(
        ({ data, changes }) => {
          entity.constants = data;
          makeChanges(changes, true);
          vscode.postMessage({
            type: 'updateEntity',
            body: JSON.stringify(entity),
          });
  ...
  Blockly.ContextMenuRegistry.registry.register({
    ...
    callback() {
      table_modal(['name', 'value'], entity.aliases).then(
        ({ data, changes }) => {
          entity.aliases = data;
          makeChanges(changes);
          vscode.postMessage({
  ...
  function generateEntity() {
    ...
    ws.getAllBlocks().forEach((b) =>
      b.inputList.forEach((c) =>
        c.fieldRow.forEach(
          (d) => d instanceof Blockly.FieldVariable && d.renderSelectedText_()
        )
      )
    );
  ...
  function generateAliases() {
    ...
    aliasesCode = Object.keys(entity.aliases)
  ...
  window.addEventListener(
    'message',
    (message) =>
      message.data.type == 'contentUpdate' &&
      (Blockly.Events.disable(),
      Blockly.serialization.workspaces.load(JSON.parse(message.data.body), ws, {
        registerUndo: false,
      }),
      Blockly.Events.enable(),
      generateEntity(),
  ...
  window.addEventListener('message', async (message) => {
    if (message.data.type == 'entity') {
      ...
      Object.keys(entity.signals).forEach(
        (s) =>
          (entity.signals[s] =
            entity.signals[s] instanceof Array
              ? entity.signals[s]
              : [entity.signals[s], ''])
      );
      ...
      constantsCode = Object.keys(entity.constants)
        .map(
          (n) =>
            '\n  constant ' +
            n +
      ...
      for (const a in libraries) {
        ...
        for (const lib in libraries[a]) {
          if (libraries[a][lib] === null) continue;
          else if (libraries[a][lib] === 'builtin') def = builtinLibs[a][lib];
          else
            def = eval(
              '((colour, generator)=>' +
                (await getFile(libraries[a][lib])) +
                ')'
      ...
      librariesCode = Object.keys(libraries)
        .map(
          (l) =>
            'library ' +
            l +
